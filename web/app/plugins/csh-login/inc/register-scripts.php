...
function cshlg_enqueue_widget_style() {
    ...
    if ( !empty($cshlg_options['font_color'])) {
        ...
        $dynamic_css .= " .login_dialog label, .login_dialog h2 { color: {$font_color} !important; } ";
    }
...
//        $sent_status = wp_mail($to, $subject, $body);
        ...
        if ($generated_pass == "on") {
            ...
            // login
            $login_data                  = array();
            $login_data['user_login']    = $register_user;
            $login_data['user_password'] = $password_register;
