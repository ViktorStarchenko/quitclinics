...
function cptapagination_default($atts) {
	...
	echo "<div id='cptapagination-content'>";
		if( $cptaQuery->have_posts() ){
			while( $cptaQuery->have_posts() ){ $cptaQuery->the_post();
				 echo "<div class='post-slider-block'>
					<p><a href='" .get_the_permalink(). "' ><figure><img alt='" .get_the_title(). "' src='" .the_post_thumbnail(). "'></figure></a></p>
					<p class='post-slider-category'>" .the_category(). "</p>
					<h6><a href='" .get_the_permalink(). "' >" .get_the_title(). "</a></h6>
					<p class='post-slider-excerpt'>" .get_the_excerpt(90). "...</p>					
				</div>";
			} wp_reset_postdata();
		}
		if($cptataxname!=""){
