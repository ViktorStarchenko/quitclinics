...
function adfoin_pabbly_settings_view( $current_tab )
{
    ...
    $nonce = wp_create_nonce( "adfoin_pabbly_settings" );
    $api_key = ( get_option( 'adfoin_pabbly_api_key' ) ? get_option( 'adfoin_pabbly_api_key' ) : "" );
    ?>

    <form name="pabbly_save_form" action="<?php 
    echo  esc_url( admin_url( 'admin-post.php' ) ) ;
    ?>"
          method="post" class="container">

        <input type="hidden" name="action" value="adfoin_save_pabbly_api_key">
    ...
    ?>"/>
        ...
        <table class="form-table">
            <tr valign="top">
                <th scope="row"> <?php 
    _e( 'Pabbly Bearer Token', 'advanced-form-integration' );
    ?></th>
                <td>
                    <input type="text" name="adfoin_pabbly_api_key"
                           value="<?php 
    echo  $api_key ;
    ?>" placeholder="<?php 
    _e( 'Please enter Bearer Token', 'advanced-form-integration' );
    ?>"
                           class="regular-text"/>
                    <p class="description" id="code-description"><?php 
    _e( 'Go to Integrations > Developer API to get Bearer Token', 'advanced-form-integration' );
    ?></a></p>
                </td>
            </tr>
        </table>
...
function adfoin_get_pabbly_list()
{
    ...
    $api_key = get_option( "adfoin_pabbly_api_key" );
    if ( !$api_key ) {
        return array();
    }
