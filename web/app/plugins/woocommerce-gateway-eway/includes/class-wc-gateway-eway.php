	...
	class WC_Gateway_EWAY extends WC_Payment_Gateway {
		...
		public function enqueue_scripts() {
			...
			if ( ! defined( 'WC_VERSION' ) ) {
				wp_register_script( 'jquery-payment', $this->plugin_url() . 'assets/js/frontend/jquery.payment' . $suffix . '.js', array( 'jquery' ), WOOCOMMERCE_GATEWAY_EWAY_VERSION, true );
			}
		...
		public function validate_checkout_values( $values, $errors ) {
			...
			foreach( $requirements as $name => $length ) {
				...
				$errors->add(
					'validation',
					sprintf(
						__( 'The value of "%s" must not exceed the length of %d characters for payments with eWay.', 'wc-eway' ),
