	...
	class WC_EWAY_API {
		...
		public function request_access_code( $order, $method = 'ProcessPayment', $trx_type = 'Purchase', $order_total = null ) {
			...
			$request = array(
				...
				'Customer'        => array(
					...
					'CompanyName' => $pre_wc_30 ? $order->billing_company : $order->get_billing_company(),
		...
		public function debug_message( $message ) {
			if ( 'on' === $this->debug_mode ) {
				WC_Gateway_EWAY::log( is_array( $message ) || is_object( $message ) ? print_r( $message, true ) : $message );
			}
		}
