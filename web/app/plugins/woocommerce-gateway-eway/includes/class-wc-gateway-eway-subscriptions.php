...
if ( ! class_exists( 'WC_Gateway_EWAY_Subscriptions' ) ) {
	class WC_Gateway_EWAY_Subscriptions extends WC_Gateway_EWAY {
		...
		public function maybe_render_subscription_payment_method( $payment_method_to_display, $subscription ) {
			$is_pre_wc_3_0  = version_compare( WC_VERSION, '3.0', '<' );
			$customer_id    = $is_pre_wc_3_0 ? $subscription->customer_user : $subscription->get_customer_id();
			$payment_method = $is_pre_wc_3_0 ? $subscription->payment_method : $subscription->get_payment_method();
			...
			$subscription_id = $is_pre_wc_3_0 ? $subscription->id : $subscription->get_id();
			$order_token_id  = get_post_meta( $subscription_id, '_eway_token_customer_id', true );
